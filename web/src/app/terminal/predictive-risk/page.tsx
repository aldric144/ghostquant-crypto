...
function PredictiveRiskEnginePageContent() {
  ...
  const safePredictions = safeArray<Prediction>(predictions)
  const safeMetrics: PredictiveMetrics = {
    avgRisk: safeNumber(metrics?.avgRisk),
    highRiskAssets: safeNumber(metrics?.highRiskAssets),
    avgConfidence: safeNumber(metrics?.avgConfidence),
    bullishPredictions: safeNumber(metrics?.bullishPredictions),
    bearishPredictions: safeNumber(metrics?.bearishPredictions),
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 p-6">
      <div className="max-w-7xl mx-auto">
        <div className="mb-8"><TerminalBackButton className="mb-4" />
          ...
          <p className="text-gray-400">AI-powered risk forecasting with probability bands</p></div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          <div className="lg:col-span-2 bg-slate-800/50 border border-cyan-500/20 rounded-lg overflow-hidden">
            ...
            <div className="max-h-[500px] overflow-y-auto">
              {safePredictions.sort((a, b) => safeNumber(b?.riskScore) - safeNumber(a?.riskScore)).map(pred => (
                <div key={pred?.id || Math.random()} className="p-4 border-b border-slate-700/50 hover:bg-slate-700/30">
                  <div className="flex items-center justify-between mb-2">
                    <div className="flex items-center gap-3"><span className="font-medium text-cyan-400 text-lg">{pred?.asset || 'Unknown'}</span><span className={`font-medium ${getTrendColor(pred?.trend || '')}`}>{(pred?.trend || 'neutral').toUpperCase()}</span></div>
            ...
            <div className="bg-slate-800/50 border border-cyan-500/20 rounded-lg p-6">
              ...
              <div className="space-y-3">
                {[['High', 70, 100, 'bg-red-500'], ['Medium', 40, 70, 'bg-yellow-500'], ['Low', 0, 40, 'bg-green-500']].map(([label, min, max, color]) => {
                  const count = safePredictions.filter(p => safeNumber(p?.riskScore) >= (min as number) && safeNumber(p?.riskScore) < (max as number)).length
                  const pct = safePredictions.length > 0 ? (count / safePredictions.length) * 100 : 0
