...
export default function StrategyBacktesterPage() {
  ...
  const getReturnColor = (r: number) => r >= 0 ? 'text-green-400' : 'text-red-400'
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 p-6">
      <div className="max-w-7xl mx-auto">
        <div className="mb-8"><h1 className="text-3xl font-bold text-cyan-400 mb-2">Strategy Backtester</h1><p className="text-gray-400">Historical strategy performance analysis and validation</p></div>
        <div className="grid grid-cols-2 md:grid-cols-6 gap-4 mb-8">
          <div className="bg-slate-800/50 border border-cyan-500/20 rounded-lg p-4"><div className="text-xs text-gray-400 mb-1">Total Backtests</div><div className="text-2xl font-bold text-cyan-400">{metrics.totalBacktests}</div></div>
          <div className="bg-slate-800/50 border border-green-500/20 rounded-lg p-4"><div className="text-xs text-gray-400 mb-1">Avg Return</div><div className={`text-2xl font-bold ${getReturnColor(metrics.avgReturn)}`}>{metrics.avgReturn >= 0 ? '+' : ''}{metrics.avgReturn.toFixed(1)}%</div></div>
          <div className="bg-slate-800/50 border border-purple-500/20 rounded-lg p-4"><div className="text-xs text-gray-400 mb-1">Avg Sharpe</div><div className="text-2xl font-bold text-purple-400">{metrics.avgSharpe.toFixed(2)}</div></div>
          <div className="bg-slate-800/50 border border-yellow-500/20 rounded-lg p-4"><div className="text-xs text-gray-400 mb-1">Avg Win Rate</div><div className="text-2xl font-bold text-yellow-400">{metrics.avgWinRate.toFixed(0)}%</div></div>
          ...
          <div className="bg-slate-800/50 border border-blue-500/20 rounded-lg p-4"><div className="text-xs text-gray-400 mb-1">Running</div><div className="text-2xl font-bold text-blue-400">{metrics.runningNow}</div></div>
        </div>
              ...
              <tbody>
                {backtests.map(bt => (
                  <tr key={bt.id} className="border-t border-slate-700/50 hover:bg-slate-700/30">
                    ...
                    <td className={`p-3 text-right font-medium ${getReturnColor(bt.totalReturn)}`}>{bt.totalReturn >= 0 ? '+' : ''}{bt.totalReturn.toFixed(1)}%</td>
                    <td className="p-3 text-right text-purple-400">{bt.sharpeRatio.toFixed(2)}</td>
                    <td className="p-3 text-right text-red-400">-{bt.maxDrawdown.toFixed(1)}%</td>
                    <td className="p-3 text-right text-yellow-400">{bt.winRate.toFixed(0)}%</td>
                    <td className="p-3 text-right text-gray-400">{bt.trades}</td>
                  </tr>
                ))}
              </tbody>
            </table>
