...
export default function AIForecastEnginePage() {
  ...
  const getChangeColor = (c: number) => c > 0 ? "text-green-400" : c < 0 ? "text-red-400" : "text-gray-400"
  ...
  const fmt = (v: number) => v >= 1000 ? `$${v.toLocaleString()}` : `$${v.toFixed(2)}`
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 p-6">
      <div className="max-w-7xl mx-auto">
        <div className="mb-8"><h1 className="text-3xl font-bold text-cyan-400 mb-2">AI Forecast Engine</h1><p className="text-gray-400">Machine learning price predictions with confidence intervals</p></div>
        <div className="grid grid-cols-2 md:grid-cols-5 gap-4 mb-8">
          <div className="bg-slate-800/50 border border-cyan-500/20 rounded-lg p-4"><div className="text-xs text-gray-400 mb-1">Forecasts</div><div className="text-2xl font-bold text-cyan-400">{metrics.totalForecasts}</div></div>
          ...
          <div className="bg-slate-800/50 border border-green-500/20 rounded-lg p-4"><div className="text-xs text-gray-400 mb-1">Bullish</div><div className="text-2xl font-bold text-green-400">{metrics.bullishForecasts}</div></div>
          <div className="bg-slate-800/50 border border-red-500/20 rounded-lg p-4"><div className="text-xs text-gray-400 mb-1">Bearish</div><div className="text-2xl font-bold text-red-400">{metrics.bearishForecasts}</div></div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          <div className="lg:col-span-2 bg-slate-800/50 border border-cyan-500/20 rounded-lg overflow-hidden">
            ...
            <div className="max-h-[500px] overflow-y-auto">
              {forecasts.sort((a, b) => Math.abs(b.change) - Math.abs(a.change)).map(f => (
                <div key={f.id} className="p-4 border-b border-slate-700/50 hover:bg-slate-700/30">
                  <div className="flex items-center justify-between mb-2">
                    ...
                    <span className={`text-xl font-bold ${getChangeColor(f.change)}`}>{f.change >= 0 ? "+" : ""}{f.change.toFixed(1)}%</span>
                  </div>
                  <div className="flex items-center justify-between text-sm">
                    <span className="text-gray-400">Current: {fmt(f.currentPrice)}</span>
                    <span className={`font-medium ${getChangeColor(f.change)}`}>Predicted: {fmt(f.predictedPrice)}</span>
                    <span className="text-purple-400">Conf: {f.confidence.toFixed(0)}%</span>
                  </div>
                </div>
              ))}
            </div>
          ...
          <div className="space-y-6">
            <div className="bg-slate-800/50 border border-cyan-500/20 rounded-lg p-6">
              ...
              <div className="flex justify-center gap-8">
                <div className="text-center"><div className="text-3xl font-bold text-green-400">{metrics.bullishForecasts}</div><div className="text-xs text-gray-400">Bullish</div></div>
                <div className="text-center"><div className="text-3xl font-bold text-red-400">{metrics.bearishForecasts}</div><div className="text-xs text-gray-400">Bearish</div></div>
              </div>
            </div>
            <div className="bg-slate-800/50 border border-cyan-500/20 rounded-lg p-6">
              <h2 className="text-lg font-semibold text-white mb-4">Top Movers</h2>
              <div className="space-y-2">
                {forecasts.sort((a, b) => Math.abs(b.change) - Math.abs(a.change)).slice(0, 5).map(f => (
                  <div key={f.id} className="flex items-center justify-between"><span className="text-cyan-400">{f.asset}</span><span className={`font-medium ${getChangeColor(f.change)}`}>{f.change >= 0 ? "+" : ""}{f.change.toFixed(1)}%</span></div>
                ))}
              </div>
            </div>
          </div>
